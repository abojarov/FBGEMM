...
BASE_LOGFILE=benchmarks_run.log
BASE_LOGFILE_RESULTS=benchmarks_run_results.log
        ...
        for output_t in fp16 fp32; do  
            for emb_dim in 128 160; do  
                for bag_size in 8 40; do  
                    ...
                    CMD="python bench/split_table_batched_embeddings_benchmark.py device --alpha $alpha --bag-size $bag_size --batch-size 131072 --embedding-dim $emb_dim --weights-precision fp16 --num-embeddings 7000000 --num-tables 2 --output-dtype $output_t --iters 300 --warmup-runs 50 --flush-gpu-cache-size-mb 80 --return-vectors True --output-dtype fp32"
                    echo ">>>>>>> Command: $CMD"
                    `$CMD`
                done  
            done  
